...
NOTE: Replace *_<name>_* and *_<domain>_*. (E.g. https://argocd-infra-server-openshift-gitops.apps.sno-<name>.<domain>)
...
oauth:
  ...
  keycloak: # Update chart values
    ...
    issuer: https://keycloak-keycloak.apps.argo-hub.<domain>/realms/myrealm-<name>
...
Log in the _argocd-infra_ instance (E.g. https://argocd-infra-server-openshift-gitops.apps.sno-<name>.<domain>)
...
- _Log out_ and _Log in_ back (E.g. https://console-openshift-console.apps.sno-<name>.<domain>). 
...
- Log in _argocd-infra_ instance console: (E.g. https://argocd-infra-server-openshift-gitops.apps.sno-<name>.<domain>)
...
- Log in *_argocd-apps_* instance console: (E.g. https://argocd-apps-server-openshift-operators.apps.sno-<name>.<domain>)
...
Then deploy an _example-app_ on *_argocd-apps_* instance (E.g. https://argocd-apps-server-openshift-operators.apps.sno-<name>.<domain>)
...
Then navigate to OpenShift _SNO_ console (E.g. https://console-openshift-console.apps.sno-<name>.<domain>) to verify those objects deployed in the _argocd-infra_ instance exist on the _SNO_ and if _Service Monitor_ is scraping your metrics properly:
...
[source, shell,subs="+macros,+attributes"]
----
oc login -u admin -p <pass> https://api.sno-<name>.<domain>:6443
...
- Log in OpenShift _SNO_ console (E.g. https://console-openshift-console.apps.sno-<name>.<domain>)
...
So we need to modify existing Application _sno-<name>-vault_ in _argocd-apps_ instance (E.g. https://argocd-apps-server-openshift-operators.apps.sno-<name>.<domain>) to use *plugin*. 
...
curl vault-vault-secrets.apps.sno-<name>.<domain>
----
