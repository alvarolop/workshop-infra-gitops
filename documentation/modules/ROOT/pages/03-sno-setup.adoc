...
Let's make some modifications in the code to start deploying _GitOps_ in the managed cluster. Open the _workshop-gitops-content-deploy_ folder and make sure all the changes are done in the correct branch _sno-<clusterID>-setup_:
...
IMPORTANT: Change *cluster-definition/sno-<clusterID>* folder as your assigned cluster as this value will be used as a parameter for ApplicationSet.
...
----
cd ~/workshop-gitops-content-deploy
E.g.: mv cluster-definition/sno-<clusterID> cluster-definition/sno-22
----
...
- Open the OpenShift _SNO_ console (E.g. https://console-openshift-console.apps.sno-<clusterID>.<domain>.opentlc.com)
...
- 3) Click *_Settings_* section  (E.g. https://argocd-infra-server-openshift-gitops.apps.sno-<clusterID>.<domain>.opentlc.com/settings)
...
- 5) Follow the same steps to verify that *_argocd-apps_* (E.g. https://argocd-apps-server-openshift-operators.apps.sno-<clusterID>.<domain>.opentlc.com/settings) instance exists and is NOT _cluster-wide_.
...
[source, shell,subs="+macros,+attributes"]
----
oc login -u admin -p <pass> https://api.sno-<clusterID>.<domain>.opentlc.com:6443
----
...
curl vault-vault-secrets.apps.sno-<clusterID>.<domain>.opentlc.com
----
